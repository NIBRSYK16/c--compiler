...
RUN_DIR="${1:-$BUILD_DIR/sample_semantic_runs}"

check_semantic_sample() {
    local sample="$1"
    local name="$2"
    local out_dir="$3"

    run_compiler_stage "semantic" "$sample" "$out_dir"
    local compiler_code=$?
    local actual_stage
    ...
    case "$name" in
        ...
        semantic_error_*)
            ...
            ;;
        *)
            if [ "$compiler_code" -eq 0 ] && [ -f "$out_dir/semantic.txt" ]; then
                record_pass "$name" "语义分析通过，已生成 semantic.txt"
            else
                record_fail "$name" "语义分析应通过，实际退出码 $compiler_code，阶段 $actual_stage"
            fi
            ;;
    esac
}
...
main() {
    ...
    for sample in "$SAMPLE_DIR"/*.sy; do
        ...
        check_semantic_sample "$sample" "$name" "$out_dir"
    done

    print_summary "semantic sample test" "$RUN_DIR"
}
