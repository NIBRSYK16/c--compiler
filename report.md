...
测试分为四类：单阶段模块测试、完整流程测试、汇编语义验证、IDE 展示测试。这样既能证明每个模块单独正确，也能证明模块组合后的编译结果与源程序语义一致。
...
这些脚本会自动遍历 `tests/` 下的样例，并检查样例是否在预期阶段成功或失败。测试重点不是只证明正确样例能通过，而是让错误样例能在正确阶段报错。

【截图占位 1：模块测试脚本运行结果】

请在此处插入 `run_sample_lexer.sh`、`run_sample_parser.sh`、`run_sample_semantic.sh`、`run_sample_ir.sh` 的终端运行截图，要求能看到 passed / failed 汇总。

【截图占位 2：完整流程测试运行结果】

请在此处插入 `scripts/run_samples.sh` 的终端运行截图，要求能看到完整流程测试的 passed / failed 汇总。

### 9.3 输出文件格式测试

为了验证输出格式符合大作业文档要求，选取一个正确样例运行：

```bash
build/c--compiler parser tests/ok_001_minimal_return.sy -o output/format_check
```

重点检查：

- `token.txt` 是否为 `单词符号<TAB><单词符号种别,单词符号内容>`。
- `reduce.txt` 是否为 `序号<TAB>栈顶符号#面临输入符号<TAB>执行动作`。
- `reduce.txt` 中执行动作是否只包含 `move`、`reduction`、`accept` 或 `error`。

【截图占位 3：输出格式检查】

请在此处插入 `token.txt` 和 `reduce.txt` 的局部截图，最好包含 token 输出前几行、规约输出前几行和末尾 `accept`。

### 9.4 汇编语义验证

为了进一步说明 IR 生成的结果确实保留了源程序语义，可以选取 `ok_003_local_decl_assign.sy`、`ok_009_function_call.sy`、`ok_015_logic_and_or.sy` 等代表性样例，把生成的 LLVM IR 交给 `clang` 生成汇编和可执行文件。

验证步骤：

```bash
build/c--compiler tests/ok_003_local_decl_assign.sy -o output/asm_check
clang output/asm_check/output.ll -o output/asm_check/a.out
./output/asm_check/a.out
echo $?
clang -S output/asm_check/output.ll -o output/asm_check/output.s
```

验证方法：

- 先根据 `.sy` 源程序手工计算预期返回值。
- 运行 `a.out`，用 `echo $?` 查看实际返回值。
- 打开 `output.s`，观察关键汇编指令是否与源程序语义一致，例如常量装载、加减乘除、函数调用、条件跳转等。

【截图占位 4：汇编语义验证】

请在此处插入源程序、`echo $?` 运行结果和 `output.s` 关键片段截图。截图说明中要写明：源程序预期返回值是多少，汇编中的关键指令如何体现该语义。

### 9.5 IDE 展示测试

IDE 测试主要验证交互展示是否合理，而不是验证编译算法本身。建议准备以下场景：

- 正确程序：展示语法高亮、右侧 token 输出、右侧 parser 输出。
- 词法错误：输入非法字符，展示编辑区错误标记和右侧错误提示。
- 语法错误：删除分号或右括号，展示语法错误提示。
- 光标联动：把光标移动到某个 token，展示左侧 token 和右侧输出的关联高亮。
- 粘贴与缩进：粘贴多行代码，展示缩进没有错乱。
...
请在此处插入 IDE 打开正确样例时的截图，要求能看到左侧编辑区和右侧两个分析面板。

【截图占位 6：IDE 错误提示展示】

请在此处插入 IDE 显示词法错误或语法错误时的截图，要求能看到错误标记或错误提示。

【截图占位 7：IDE 光标联动展示】

请在此处插入光标选中某个 token 后，左右区域同步高亮的截图。
